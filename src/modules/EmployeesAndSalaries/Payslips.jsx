...
export function Payslips() {
  const { data: payslipsData, addItem, updateItem, deleteItem, replaceAll } = useERPStorage('payslips');
  ...
  const findDueInstallments = (employeeId, periodStart, periodEnd) => {
    ...
    loansData.forEach(loan => {
      if (loan.employeeId === employeeId && loan.schedule && loan.schedule.length > 0) {
  ...
  const calculateOvertimeRate = (basicSalary, allowances) => {
    ...
    const totalMonthly = parseFloat(basicSalary || 0) + parseFloat(allowances || 0);
  ...
  const calculateSalaryBasedOnAttendance = (employeeId, periodStart, periodEnd, basicSalary) => {
  ...
  const calculateNetPay = (basicSalary, allowances, deductions, employeeId, periodStart, periodEnd, overtimeHours, overtimeRate, penalties, rewards, insurance) => {
    ...
    const deduct = deductions.reduce((sum, d) => sum + (parseFloat(d.amount) || 0), 0);
    ...
    return salaryBasedOnAttendance + allow + overtimePay + rewardsAmount + fridayBonus - deduct - penaltiesAmount - insuranceAmount;
  };
  ...
  useEffect(() => {
    if (formData.employeeId && formData.periodStart && formData.periodEnd && !editingItem) {
      ...
      if (employee) {
        ...
        const totalOvertimeHours = getOvertimeHours(formData.employeeId, formData.periodStart, formData.periodEnd);
        ...
        setFormData(prev => ({
          ...
          overtimeHours: totalOvertimeHours > 0 ? totalOvertimeHours.toFixed(1) : prev.overtimeHours || '',
          overtimeRate: overtimeRate > 0 ? overtimeRate.toFixed(2) : prev.overtimeRate || '',
        }));
      }

      // إزالة جميع الخصومات التلقائية (السلف والغياب) لإعادة حسابها
      const manualDeductions = formData.deductions.filter(d => d.source === 'manual');
      
      // إضافة خصومات السلف
      const dueInstallments = findDueInstallments(formData.employeeId, formData.periodStart, formData.periodEnd);
      ...
      const fridaysInPeriod = getFridaysInPeriod(formData.periodStart, formData.periodEnd);
      const fridayAttendanceDays = getFridayAttendanceDays(formData.employeeId, formData.periodStart, formData.periodEnd);
      
      // حساب أيام الحضور (بما في ذلك الجمعة)
      const totalAttendanceDays = getAttendanceDays(formData.employeeId, formData.periodStart, formData.periodEnd);
      ...
      if (employee) {
        const basicSalary = parseFloat(formData.basicSalary || employee.salary || 0);
        const fixedAllowance = parseFloat(formData.allowances || employee.fixedAllowance || 0);
  ...
  }, [formData.employeeId, formData.periodStart, formData.periodEnd, formData.basicSalary, formData.allowances, employees, loansData, attendanceData, editingItem]);
  ...
  }, [formData.basicSalary, formData.allowances, formData.deductions, formData.employeeId, formData.periodStart, formData.periodEnd, formData.overtimeHours, formData.overtimeRate, formData.penalties, formData.rewards, formData.insurance]);

  const handleSubmit = (e) => {
    e.preventDefault();
    
    if (!formData.payslipId || !formData.employeeId || !formData.periodStart || !formData.periodEnd) {
        ...
        if (ded.source === 'erp_loans' && ded.loanId) {
          ...
          if (loan && loan.schedule && ded.scheduleIndex !== undefined) {
            ...
            if (schedule[ded.scheduleIndex] && !schedule[ded.scheduleIndex].paid) {
              ...
              const newRemaining = Math.max(0, loan.remainingAmount - ded.amount);
              updateLoan(loan.loanId, {
                schedule: schedule,
                remainingAmount: newRemaining,
              }, 'loanId');
  ...
  const handleImport = (importedData, mode) => {
    try {
      const processedData = importedData.map(item => {
        const deductions = item.deductionsJson ? JSON.parse(item.deductionsJson) : [];
        return {
          ...
          overtimeHours: parseFloat(item.overtimeHours || item['overtimeHours'] || 0),
          overtimeRate: parseFloat(item.overtimeRate || item['overtimeRate'] || 0),
      ...
      if (mode === 'replace') {
        ...
        processedData.forEach(item => {
          const existing = payslipsData.find(p => p.payslipId === item.payslipId);
          if (existing) {
    ...
    printWindow.document.write(`
      <html dir="rtl">
        ...
        <body>
          ...
          <p><strong>الموظف:</strong> ${employee ? employee.name : item.employeeId}</p>
          <p><strong>الفترة:</strong> ${item.periodStart} إلى ${item.periodEnd}</p>
          <table>
            <tr><th>المرتب الأساسي</th><td>${item.basicSalary} ج.م</td></tr>
            <tr><th>البدلات</th><td>${item.allowances || 0} ج.م</td></tr>
            ${item.overtimeHours > 0 ? `<tr><th>الساعات الإضافية</th><td>${item.overtimeHours} × ${item.overtimeRate || 0} = ${(item.overtimeHours * (item.overtimeRate || 0)).toFixed(2)} ج.م</td></tr>` : ''}
            ${item.rewards > 0 ? `<tr><th>مكافآت</th><td>${item.rewards} ج.م</td></tr>` : ''}
            <tr><th>الخصومات (أقساط السلف + غياب)</th><td>${item.deductions.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0).toFixed(2)} ج.م</td></tr>
            ${item.penalties > 0 ? `<tr><th>جزاءات</th><td>${item.penalties} ج.م</td></tr>` : ''}
            ${item.insurance > 0 ? `<tr><th>تأمين</th><td>${item.insurance} ج.م</td></tr>` : ''}
          ...
          ${item.deductions.length > 0 ? `
            ...
            <table>
              ...
              ${item.deductions.map(d => `<tr><td>${d.type}</td><td>${d.amount} ج.م</td></tr>`).join('')}
            </table>
          ` : ''}
        </body>
      </html>
  ...
  const addManualDeduction = () => {
    setFormData(prev => ({
      ...prev,
      deductions: [...prev.deductions, { type: '', amount: '', source: 'manual' }],
    }));
  };
  ...
  const columns = [
    ...
    { key: 'basicSalary', label: 'المرتب الأساسي', render: (val) => `${val} ج.م` },
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">المرتب الأساسي</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">البدلات</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">مكافآت</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">جزاءات</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">تأمين</label>
              <input
                type="number"
          ...
          {formData.employeeId && formData.periodStart && formData.periodEnd && (
            <div className="bg-sky-50 border border-sky-200 rounded-lg p-4">
              <h4 className="font-semibold text-gray-800 mb-2">معلومات الحضور:</h4>
              {(() => {
                const totalAttendanceDays = getAttendanceDays(formData.employeeId, formData.periodStart, formData.periodEnd);
                const startDate = new Date(formData.periodStart);
                const year = startDate.getFullYear();
                ...
                const absentDeductionAmount = absentDays > 0 ? (dailySalary * absentDays).toFixed(2) : 0;
                const fridayBonusAmount = fridayAttendanceDays > 0 ? (dailySalary * fridayAttendanceDays).toFixed(2) : 0;
                
                return (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600">إجمالي أيام الشهر:</span>
                        <span className="font-semibold text-gray-800 mr-2 block">{totalDaysInMonth} يوم</span>
                      </div>
                      <div>
                        <span className="text-gray-600">أيام الجمعة (إجازة):</span>
                        <span className="font-semibold text-blue-600 mr-2 block">{fridaysInPeriod.length} يوم</span>
                      </div>
                      <div>
                        <span className="text-gray-600">أيام الحضور (غير الجمعة):</span>
                        <span className="font-semibold text-emerald-600 mr-2 block">{nonFridayAttendanceDays} يوم</span>
                      </div>
                      <div>
                        <span className="text-gray-600">أيام الغياب (قابلة للخصم):</span>
                        <span className="font-semibold text-rose-600 mr-2 block">{absentDays} يوم</span>
                      </div>
                      {fridayAttendanceDays > 0 && (
                        <div className="col-span-2 md:col-span-4">
                          <span className="text-gray-600">حضور أيام الجمعة (مكافأة):</span>
                          <span className="font-semibold text-emerald-600 mr-2">{fridayAttendanceDays} يوم</span>
                        </div>
                      )}
                    </div>
                    <div className="bg-white rounded p-3 border border-gray-200">
                      <div className="text-xs text-gray-600 space-y-1">
                        <div>الراتب الشهري: (الراتب الأساسي {basicSalary} + الحافز {fixedAllowance}) = {totalMonthlySalary.toFixed(2)} ج.م</div>
                        <div>الراتب اليومي: {totalMonthlySalary.toFixed(2)} ÷ 30 = {dailySalary.toFixed(2)} ج.م</div>
                        {absentDays > 0 && (
                          <div className="font-semibold text-rose-600">خصم الغياب (أيام غير الجمعة): {dailySalary.toFixed(2)} × {absentDays} = {absentDeductionAmount} ج.م</div>
                        )}
                        {fridayAttendanceDays > 0 && (
                          <div className="font-semibold text-emerald-600">مكافأة حضور الجمعة: {dailySalary.toFixed(2)} × {fridayAttendanceDays} = {fridayBonusAmount} ج.م</div>
                        )}
                        {(() => {
                          const totalOvertime = getOvertimeHours(formData.employeeId, formData.periodStart, formData.periodEnd);
                          const overtimeRate = calculateOvertimeRate(basicSalary, fixedAllowance);
                          if (totalOvertime > 0) {
                            return (
                              <div className="font-semibold text-emerald-600">
                                الساعات الإضافية: {totalOvertime.toFixed(1)} ساعة × {overtimeRate.toFixed(2)} (الراتب ÷ 30 ÷ 8) = {(totalOvertime * overtimeRate).toFixed(2)} ج.م
                              </div>
                            );
                          }
                          return null;
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{parseFloat(formData.basicSalary || 0).toFixed(2)} ج.م</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">البدلات:</span>
                  <span className="font-semibold text-emerald-600">+{parseFloat(formData.allowances || 0).toFixed(2)} ج.م</span>
                </div>
                {parseFloat(formData.overtimeHours || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">الساعات الإضافية ({formData.overtimeHours} × {formData.overtimeRate || 0}):</span>
                    <span className="font-semibold text-emerald-600">+{(parseFloat(formData.overtimeHours || 0) * parseFloat(formData.overtimeRate || 0)).toFixed(2)} ج.م</span>
                  </div>
                )}
                {parseFloat(formData.rewards || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">مكافآت:</span>
                    <span className="font-semibold text-emerald-600">+{parseFloat(formData.rewards || 0).toFixed(2)} ج.م</span>
                  </div>
                )}
                {(() => {
                  const fridayAttendanceDays = getFridayAttendanceDays(formData.employeeId, formData.periodStart, formData.periodEnd);
                  ...
                  if (fridayBonus > 0) {
                    return (
                      <div className="flex justify-between">
                        <span className="text-gray-600">مكافأة حضور الجمعة ({fridayAttendanceDays} يوم):</span>
                        <span className="font-semibold text-emerald-600">+{fridayBonus.toFixed(2)} ج.م</span>
                      </div>
                    );
                  }
                  return null;
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-rose-600">-{formData.deductions.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0).toFixed(2)} ج.م</span>
                </div>
                {parseFloat(formData.penalties || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">جزاءات:</span>
                    <span className="font-semibold text-rose-600">-{parseFloat(formData.penalties || 0).toFixed(2)} ج.م</span>
                  </div>
                )}
                {parseFloat(formData.insurance || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">تأمين:</span>
                    <span className="font-semibold text-rose-600">-{parseFloat(formData.insurance || 0).toFixed(2)} ج.م</span>
                  </div>
                )}
              </div>
            </div>
            <div className="flex justify-between items-center border-t pt-3">
              <span className="font-semibold text-gray-800 text-lg">الصافي بعد جميع الخصومات:</span>
              <span className="text-2xl font-bold text-sky-600">{formData.netPay} ج.م</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold">{getEmployeeName(viewingItem.employeeId)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">الفترة</p>
                <p className="font-semibold">{viewingItem.periodStart} إلى {viewingItem.periodEnd}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-semibold">{viewingItem.allowances || 0} ج.م</p>
              </div>
              {viewingItem.overtimeHours > 0 && (
                <div>
                  <p className="text-sm text-gray-600">الساعات الإضافية</p>
                  <p className="font-semibold text-emerald-600">
                    {viewingItem.overtimeHours} ساعة × {viewingItem.overtimeRate || 0} = {(viewingItem.overtimeHours * (viewingItem.overtimeRate || 0)).toFixed(2)} ج.م
                  </p>
                </div>
              )}
              {viewingItem.rewards > 0 && (
                <div>
                  ...
                  <p className="font-semibold text-emerald-600">+{viewingItem.rewards || 0} ج.م</p>
                </div>
              )}
              <div>
                <p className="text-sm text-gray-600">إجمالي الخصومات (أقساط السلف + غياب)</p>
                <p className="font-semibold text-rose-600">
                  {viewingItem.deductions.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0).toFixed(2)} ج.م
                </p>
              </div>
              {viewingItem.penalties > 0 && (
                <div>
                  <p className="text-sm text-gray-600">جزاءات</p>
                  <p className="font-semibold text-rose-600">-{viewingItem.penalties || 0} ج.م</p>
                </div>
              )}
              {viewingItem.insurance > 0 && (
                <div>
                  <p className="text-sm text-gray-600">تأمين</p>
                  <p className="font-semibold text-rose-600">-{viewingItem.insurance || 0} ج.م</p>
                </div>
              )}
            </div>
            ...
            <div className="border-t pt-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-sky-600">{viewingItem.netPay} ج.م</span>
              </div>
            </div>
            <div className="flex gap-3 justify-end pt-4">
              <button
